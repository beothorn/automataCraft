...
- Minecraft 1.21.1 client or dedicated server
...
3. Run `./gradlew clean build` to compile and produce `build/libs/automata-1.21.1-9.0.0.0.jar`.

## Installing the mod
1. Install NeoForge 21.10.55-beta for Minecraft 1.21.1.
...
2. In the Gradle tool window, run `Tasks > neoforgegradle runs > genIntellijRuns` if IntelliJ does not automatically create run configurations.
3. Use the generated `Run Client` or `Run Server` configuration to start a debug session. Place breakpoints as needed.
